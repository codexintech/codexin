...
function posttype_update_messages( $messages ) {
	...
	foreach ( $post_types as $post_type => $post_object ) {
		$messages[ $post_type ] = array(
			...
			5  => array( 'revision' ) ? sprintf( '%s ' . __( 'restored to revision from ', 'codexin' ) . ' %s', $post_object->labels->singular_name, wp_post_revision_title( (int) array( 'revision' ), false ) ) : false,
